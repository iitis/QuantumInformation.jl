@testset verbose=true "MatrixBases" begin
    ...
    @testset verbose=true "HermitianBasisIterator" begin
        ...
        @test [tr(m[i]' * m[j]) for i in 1:d, j in 1:d] ≈ Matrix{Float64}(I, d, d)
    end
    ...
    @testset verbose=true "representchannel, combinechannel" begin
        (idim, odim) = (2, 4)
        A =
            reshape(collect(1:16), idim * odim, idim) *
            reshape(collect(1:16), idim * odim, idim)'
        ...
        A =
            reshape(collect(1:64), idim * odim, idim * odim) *
            reshape(collect(1:64), idim * odim, idim * odim)' +
            Matrix{Float64}(I, idim * odim, idim * odim)
    ...
    @testset verbose=true "hermitainbasis" begin
        @test hermitianbasis(Matrix{Float32}, 2) ==
              HermitianBasisIterator{Matrix{Float32}}(2)
    ...
    @testset verbose=true "Double64 Support" begin
        ...
        @test [tr(m[i]' * m[j]) for i in 1:d, j in 1:d] ≈ Matrix{Double64}(I, d, d) atol=1e-15
    ...
    @testset verbose=true "Sparse Support" begin
        ...
        @test [real(tr(m[i]' * m[j])) for i in 1:(d ^ 2), j in 1:(d ^ 2)] ≈ Matrix{Float64}(I, d^2, d^2)
        ...
        @test [real(tr(m_chan[i]' * m_chan[j])) for i in 1:n, j in 1:n] ≈ Matrix{Float64}(I, n, n)
