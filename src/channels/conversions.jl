...
function Base.convert(
    ...
    return Stinespring{T1}(convert(T1, m), ko.idim, ko.odim)
end
...
"""
function Base.convert(
    ::Type{DynamicalMatrix{T1}},
    Φ::KrausOperators{T2},
) where {T1 <: AbstractMatrix{<:Number}, T2 <: AbstractMatrix{<:Number}}
    m = sum(res(k) * res(k)' for k in Φ.matrices)
    return DynamicalMatrix{T1}(convert(T1, m), Φ.idim, Φ.odim)
end
...
"""
function Base.convert(
    ::Type{KrausOperators{T1}},
    Φ::SuperOperator{T2},
) where {T1 <: AbstractMatrix{<:Number}, T2 <: AbstractMatrix{<:Number}}
    return convert(KrausOperators{T1}, convert(DynamicalMatrix{T2}, Φ))
end
...
function Base.convert(
    ...
    return convert(Stinespring{T1}, convert(KrausOperators{T1}, Φ))
end
...
function Base.convert(
    ::Type{KrausOperators{T1}},
    Φ::DynamicalMatrix{T2},
) where {T1 <: AbstractMatrix{<:Number}, T2 <: AbstractMatrix{<:Number}}
    isnumbernotint(eltype(T1)) ? () :
    throw(ArgumentError("Kraus operators element type must be subtype of Real or Complex"))
    ...
    return KrausOperators{T1}(v, Φ.idim, Φ.odim)
end
...
function Base.convert(
    ...
    return KrausOperators{T1}(T1[convert(T1, Φ.matrix)], Φ.idim, Φ.odim)
end

function Base.convert(
    ::Type{KrausOperators{T1}},
    Φ::IdentityChannel{T2},
) where {
    T1 <: AbstractMatrix{N1},
    T2 <: AbstractMatrix{N2},
} where {N1 <: Number, N2 <: Number}
    N = promote_type(N1, N2)
    # KrausOperators{T1}(T1[eye(N, Φ.idim)], Φ.idim, Φ.odim)
    return KrausOperators{T1}(T1[Matrix{N}(I, Φ.idim, Φ.idim)], Φ.idim, Φ.odim)
end

function Base.convert(
    ::Type{KrausOperators{T1}},
    Φ::POVMMeasurement{T2},
) where {T1 <: AbstractMatrix{<:Number}, T2 <: AbstractMatrix{<:Number}}
    # TODO : Verify!
    ...
    for chin in
        ...
        @eval begin
            function Base.convert(
                ...
                return convert($chout{T1}, convert(KrausOperators{T1}, Φ))
            end
        end
    end
end
