...
function Base.kron(
    ...
    T <: AbstractQuantumOperation{M},
    T1 <: AbstractQuantumOperation{M1},
    T2 <: AbstractQuantumOperation{M2},
} where {
    M <: AbstractMatrix{<:Number},
    M1 <: AbstractMatrix{<:Number},
    M2 <: AbstractMatrix{<:Number},
}
...
function Base.kron(
    ...
    M = promote_type(M1, M2)
    return kron(KrausOperators{M}, Φ1, Φ2)
end
...
function Base.kron(
    ...
    Φ1::UnitaryChannel,
    Φ2::UnitaryChannel,
) where {T <: AbstractQuantumOperation{M}} where {M <: AbstractMatrix{<:Number}}
...
function Base.kron(
    Φ1::UnitaryChannel{M1},
    Φ2::UnitaryChannel{M2},
) where {M1 <: AbstractMatrix{<:Number}, M2 <: AbstractMatrix{<:Number}}
    M = promote_type(M1, M2)
    return kron(UnitaryChannel{M}, Φ1, Φ2)
end
...
function compose(
    ...
    M = promote_type(M1, M2)
    return compose(UnitaryChannel{M}, Φ1, Φ2)
end

function compose(
    ::Type{UnitaryChannel{M}},
    Φ1::UnitaryChannel{M1},
    Φ2::UnitaryChannel{M2},
) where {
    M <: AbstractMatrix{<:Number},
