...
"""
function norm_diamond(Φ::AbstractQuantumOperation, method::Symbol=:primal, eps=1e-7)
    return norm_diamond(Φ, Val(method), eps)
end
...
function norm_diamond(Φ::DynamicalMatrix, ::Val{:dual}, eps)
    ...
    constraints = [isposdef(Y₀), isposdef(Y₁), isposdef(Z+Z')]
...
"""
function diamond_distance(
    Φ1::DynamicalMatrix{T},
    Φ2::DynamicalMatrix{T},
    args...,
    ...
    return norm_diamond(Φ, args...)
end

function diamond_distance(
    Φ1::AbstractQuantumOperation{T},
    Φ2::AbstractQuantumOperation{T},
    args...,
) where {T <: AbstractMatrix{<:Number}}
    return diamond_distance(
        convert(DynamicalMatrix{T}, Φ1),
        convert(DynamicalMatrix{T}, Φ2),
        args...,
    )
end
