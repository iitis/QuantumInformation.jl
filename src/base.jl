...
function ket(::Type{T}, val::Int, dim::Int) where {T <: AbstractVector{<:Number}}
    ...
    1 <= val <= dim ? () :
    throw(ArgumentError("Label have to be smaller than vector dimension"))
    ψ = T(undef, dim)
...
function ket(::Type{<:AbstractSparseVector{T}}, val::Int, dim::Int) where {T <: Number}
    return sparsevec([val], [one(T)], dim)
end
...
function ketbra(
    ...
    idim > 0 && odim > 0 ? () :
    throw(ArgumentError("Matrix dimension has to be nonnegative"))
    1 <= valk <= idim && 1 <= valb <= odim ? () :
    throw(ArgumentError("Ket and bra labels have to be smaller than operator dimension"))
    ρ = T(undef, odim, idim)
...
function ketbra(
    ::Type{<:AbstractSparseMatrix{T}},
    valk::Int,
    valb::Int,
    idim::Int,
    odim::Int,
) where {T <: Number}
    return sparse([valb], [valk], [one(T)], odim, idim)
end
...
function ketbra(
    ...
    return ketbra(T, valk, valb, dim, dim)
end
function ketbra(::Type{T}, valk::Int, valb::Int, dim::Int) where {T <: Number}
    return ketbra(Matrix{T}, valk, valb, dim)
end
...
# Return outer product \$|valk\\rangle\\langle vakb|\$ of states \$|valk\\rangle\$ and \$|valb\\rangle\$.
"""
function ketbra(valk::Int, valb::Int, idim::Int, odim::Int)
    return ketbra(Matrix{ComplexF64}, valk, valb, idim, odim)
end
...
"""
function res(ρ::AbstractMatrix{<:Number})
    @cast x[(j, i)] := ρ[i, j] i in 1:size(ρ, 1), j in 1:size(ρ, 2)
end
...
function werner_state(d::Int, α::Number)
    ...
    return α * proj(max_entangled(complex(typeof(α)), d)) + (1 - α) * max_mixed(d)
end
...
function permutesystems(
    ...
    return sparse(new_I, new_J, V, size(ρ, 1), size(ρ, 2))
end
